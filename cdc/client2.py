...
class MyComponent(ApplicationSession):
    ...
    def onJoin(self, details):
        ...
        def on_event(*args, **kwargs):
            ...
            self.log.info('event received for "{topic}": {args} {kwargs}', topic=details.topic, args=args, kwargs=kwargs)

        try:
            yield self.subscribe(on_event, u'com.crossbario.cdc.api.', SubscribeOptions(match=u'prefix', details_arg=u'details'))
        except:
            self.log.failure()
            self.leave()
        else:
            self.log.info('listening to CDC events ..')
